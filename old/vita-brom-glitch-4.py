...
# glitch cycle
open('glitch_out_4.csv', 'w').close()
f = open('glitch_out_4.csv', 'ab')
writer = csv.writer(f)
target.init()
    ...
    for width in xrange(MIN_WIDTH, MAX_WIDTH+1, WIDTH_STEP):
        ...
        while timeout > 0:
            while mmc.count() > 0:
                ...
                if pkt.is_req and pkt.cmd == MMCPacket.Cmd.READ_SINGLE_BLOCK:
                    num_reads += 1
                if pkt.is_req:
                    if state == States.STARTUP:
                        ...
                        if pkt.cmd == MMCPacket.Cmd.GO_IDLE_STATE:
                            ...
                            if pkt.content == 0x606B:
                                state = States.EXPECTED_READ
                            else:
                                state = States.UNEXPECTED_READ
                        else:
                            state = States.UNEXPECTED_PACKET
                if FAST_MODE and state.value > States.UNEXPECTED_READ.value:
                    timeout = -1
                    break
            else:
                time.sleep(0.1)
        ...
        data = [offset, width, state, num_reads]
        print(data)
