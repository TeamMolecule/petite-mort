#!/usr/bin/env python2
## 
## vita-petite-mort-exception-deux.py -- glitch enable brom + run payload
##
...
mmctrigger.setTriggerData('0x7fff')
...
class PetiteMort:
        ...
        while tries > 0:
            for offset in xrange(P2_MIN_OFFSET, P2_MAX_OFFSET+1, P2_OFFSET_STEP):
                ...
                for width in xrange(P2_MIN_WIDTH, P2_MAX_WIDTH+1, P2_WIDTH_STEP):
                    ...
                    if state == States.CODE_EXEC:
                        return True
                    #if state == States.NOTHING_SEEN:
                    #    return False
            tries -= 1
        return False
    ...
    def dumpPayload(self, path=None):
        ...
        while offset < 0x40000:
            count = ser.inWaiting()
            while count > 0 and offset < 0x40000:
                timeout = PAYLOAD_TIMEOUT
                dat = ser.read(count, 0)
                self.queue.extend(dat)
                if f:
